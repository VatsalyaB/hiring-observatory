...
export function greenhouseUrl(boardId, { content = true } = {}) {
  const url = new URL(`https://boards-api.greenhouse.io/v1/boards/${encodeURIComponent(boardId)}/jobs`);
  if (content) url.searchParams.set('content', 'true');
  return url.toString();
}

export default async function collectGreenhouse({ employer, fetchPage, qualification = false, content = true }) {
  assertAtsEmployer(employer, 'greenhouse');
  const page = await readAtsJsonPage(fetchPage, greenhouseUrl(employer.board_id, { content }), 1);
